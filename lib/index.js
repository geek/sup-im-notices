...
// temporary flag to skip all status.io processing
var skipStatus = process.env.SKIP_STATUS;
  ...
  jira
    ...
    .then(function(issue) {
      ...
      var ctx = {
        ...
        issue: issue.fields,
        current_status_txt: ' ' + issue.fields[conf.STATUS]
        ...
        details: {
          ...
          incident_owner: (issue.fields[conf.INCIDENT_OWNER]) ?
            issue.fields[conf.INCIDENT_OWNER].displayName : null,
        },
        msg: {
  ...
  request.get({
    ...
    headers: ctx.statusio.headers
      ...
      FAIL('Status.io', 'The JIRA has an "External Notice Link" but it ' + 
        'could not be found.');
      console.error('Consider deleting or updating the link on the JIRA.');
      console.error('Continuing, but no actions will be taken on Status.io');
  ...
  jira
    ...
    .then(function(issue) {
      if (issue.fields.updated !== ctx.issue.updated) {
        console.log('The notice preview is stale. Please re-run im-notices ' + 
          'for the latest data.');
