  ...
  request.get({
    ...
    headers: ctx.statusio.headers
      ...
      if (!foundComponent) {
        ...
        console.error('No notice will be posted on Status.io, you should' +
      ...
      if (ctx.details.location === 'None') {
          WARN('Status.io', 'The Location is currently set to "None" on ' +
            'the JIRA.');
          console.error('No notice will be posted on Status.io, you should' +
            ' add a valid location to the JIRA.');
          return next(null, ctx);
      }
      ...
      if (ctx.issue[ctx.conf.LOCATION].length) {
        ...
        for (var i = 0; i < locations.length; i++) {
          for (var j = 0; j < foundComponent.containers.length; j++) {
        ...
        if (missingLocations.length) {
          WARN('Status.io', 'The following Locations could not be found for ' +
            '"' + ctx.issue.issuetype.name + '": ' +
            missingLocations.join(', '));
          console.error('Valid locations for this type are: ' +
            components.join(', '));
          console.error('You should correct the JIRA before proceeding.');
        }
